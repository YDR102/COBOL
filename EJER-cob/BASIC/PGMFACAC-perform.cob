      ...
      *COPY CPYFACA3.
       01  REG-FACTURA-INCIDE.
           ...
           05 FECHA-FACTURA.
                ...
                10  FILLER                        PIC X(01) VALUE '-'.
                10  DIA                           PIC X(02).
           05 IMPORTE-FACTURA                     PIC 9(07)V9(02).
      *
      ******************************************************************
      ...
      *
           PERFORM 2400-GRABAR-TABLA-SALIDA
              THRU 2400-GRABAR-TABLA-SALIDA-EXIT
      *
           PERFORM 3000-FIN
              THRU 3000-FIN-EXIT
      *
      ...
      *
       2000-PROCESO.
      *
           PERFORM 2100-BUS-DIV-EN-TB
              THRU 2100-BUS-DIV-EN-TB-EXIT
      *
           IF SW-SI-FIN-TABLA
              PERFORM 2300-ESCRIBIR-FINCIDE
                 THRU 2300-ESCRIBIR-FINCIDE-EXIT
           ELSE
              MOVE ID-CLIENTE-IN TO ID-CLI-TABLA (TB-INDICE)
              MOVE NOMBRE-CLIENTE-IN TO NOMBRE-CLI-TABLA (TB-INDICE)
              ADD IMPORTE-FACTURA-IN TO IMPORTE-TOTAL-TABLA (TB-INDICE)
           END-IF
      *
           PERFORM 9000-LEER-FENTRADA
              THRU 9000-LEER-FENTRADA-EXIT
      ...
      *
           ...
           PERFORM VARYING TB-INDICE FROM CT-1 BY 1
               UNTIL TB-INDICE > 5 OR SW-SI-FIN-TABLA

               IF ID-CLI-TABLA (TB-INDICE) = ID-CLIENTE-IN
                   ADD IMPORTE-FACTURA-IN
                   TO IMPORTE-TOTAL-TABLA (TB-INDICE)
                   SET SW-SI-FIN-TABLA TO TRUE
               ELSE
                   IF ID-CLI-TABLA (TB-INDICE) = SPACES
                       MOVE ID-CLIENTE-IN
      ...
      *
           MOVE ID-CLIENTE-IN       TO ID-CLIENTE
           MOVE NOMBRE-CLIENTE-IN   TO NOMBRE-CLIENTE
           MOVE FECHA-FACTURA-IN    TO FECHA-FACTURA
      *
      ...
      *
      ******************************************************************
      ** 2400-GRABAR-TABLA-SALIDA                                     **
      ** RECORRE LA TABLA Y ESCRIBE LOS REGISTROS EN FSALIDA          **
      ******************************************************************
      *
       2400-GRABAR-TABLA-SALIDA.
      *
           PERFORM VARYING TB-INDICE FROM CT-1 BY 1
              UNTIL TB-INDICE > 5
      *
              IF ID-CLI-TABLA (TB-INDICE) NOT = SPACES
                 MOVE ID-CLI-TABLA (TB-INDICE)
                 TO ID-CLIENTE-OUT
      *
                 MOVE NOMBRE-CLI-TABLA (TB-INDICE)
                 TO NOMBRE-CLIENTE-OUT
      *
                 MOVE IMPORTE-TOTAL-TABLA (TB-INDICE)
                 TO IMPORTE-ACOMULADO-OUT
      *
